...
void Brain::handleCooperation() {
    ...
    double cost_threshold = 3.0;

    get_parameter("strategy.cooperation.ball_control_cost_threshold", BALL_CONTROL_COST_THRESHOLD);

    if (data->tmMyCost < tmMinCost && tmMinCost > BALL_CONTROL_COST_THRESHOLD) {//比队友低且队友成本都高时我是leader
        data->tmImLead = true;
...
void Brain::updateCostToKick() {
    ...
    data->tmMyCost = lastCost * 0.3 + cost * 0.7;
